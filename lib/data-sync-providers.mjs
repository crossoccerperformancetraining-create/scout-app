...
const TSDB_FREE_KEY=process.env.THESPORTSDB_API_KEY||'123';
const tsdbBase=()=>`https://www.thesportsdb.com/api/v1/json/${encodeURIComponent(TSDB_FREE_KEY)}`;
function tsdbRows(d,key){const v=d?.[key];return Array.isArray(v)?v:[]}
function tsdbPlayerRow(p={}){return{id:p.idPlayer||p.id||'',name:p.strPlayer||p.name||'',photo:p.strCutout||p.strThumb||'',team:p.strTeam||'',position:p.strPosition||'',nationality:p.strNationality||'',birthDate:p.dateBorn||p.dateBornPlayer||'',age:p.strAge||''}}
function tsdbStatField(name){const k=fold(name);if(/appearances|matches|games/.test(k))return'games';if(/starts|starting xi|starting eleven/.test(k))return'starts';if(/mins played|minutes played|minutes/.test(k))return'minutes';if(/^goals?$/.test(k))return'goals';if(/assists?/.test(k))return'assists';if(/yellow/.test(k)&&/card/.test(k))return'yellowCards';if(/red/.test(k)&&/card/.test(k))return'redCards';return''}
function tsdbGroups(rows=[]){const grouped=new Map();for(const row of rows){const field=tsdbStatField(row.strStatistic);if(!field)continue;const season=clean(row.strSeason)||'Atual',competition=clean(row.strLeague)||'Competição',club=clean(row.strTeam),key=[season,competition,club].join('||');if(!grouped.has(key))grouped.set(key,{seasonLabel:season,competition,club,values:{},meta:{teamId:row.idTeam||'',leagueId:row.idLeague||''}});const raw=String(row.strValue??'').replace(',','.').replace(/[^0-9.\-]/g,''),value=Number(raw);if(Number.isFinite(value))grouped.get(key).values[field]=Math.max(0,value)}return[...grouped.values()]}
export async function theSportsDB({playerId,season}){
  const id=clean(playerId);if(!id)throw new Error('ID TheSportsDB obrigatório.');
  const [pd,sd]=await Promise.all([jfetch(`${tsdbBase()}/lookupplayer.php?id=${encodeURIComponent(id)}`),jfetch(`${tsdbBase()}/lookupplayerstats.php?id=${encodeURIComponent(id)}`)]);
  const profile=tsdbRows(pd,'players')[0]||tsdbRows(pd,'player')[0];if(!profile)throw new Error('Atleta não encontrado no TheSportsDB.');
  let groups=tsdbGroups(tsdbRows(sd,'playerstats'));const wanted=clean(season);if(wanted){const exact=groups.filter(g=>fold(g.seasonLabel).includes(fold(wanted)));if(exact.length)groups=exact}
  const p=tsdbPlayerRow(profile),values=groups.length?aggregate(groups,wanted||groups[0]?.seasonLabel||'Atual',p.team):null;
  return{ok:true,provider:'thesportsdb',externalId:String(id),season:wanted||groups[0]?.seasonLabel||'Atual',player:{id:p.id,name:p.name,nationality:p.nationality,birthDate:p.birthDate,photo:p.photo,position:p.position,team:p.team,teamId:profile.idTeam||'',teamLogo:'',teamIsCurrent:!!p.team},groups,aggregate:values||undefined,fetchedAt:Date.now(),coverage:groups.length?'stats':'profile-only'};
}
...
export async function fetchPlayerStats(input){switch(input.provider){case'api-football':return apiFootball(input);case'thesportsdb':return theSportsDB(input);case'sportmonks':return sportmonks(input);case'football-data':return footballData(input);default:throw new Error('Provedor não suportado.')}}
...
export async function searchPlayers({provider,name,season}){
  ...
  if(provider==='thesportsdb'){
    const terms=uniqueSearchTerms(name),found=new Map();
    for(const term of terms){const d=await jfetch(`${tsdbBase()}/searchplayers.php?p=${encodeURIComponent(term)}`);const rows=[...tsdbRows(d,'player'),...tsdbRows(d,'players')];for(const x of rows){if(x?.strSport&&fold(x.strSport)!=='soccer'&&fold(x.strSport)!=='football')continue;const row=tsdbPlayerRow(x);if(row.id&&!found.has(String(row.id)))found.set(String(row.id),row)}if(found.size)break}
...
function providerConfigured(provider){if(provider==='api-football')return!!process.env.API_FOOTBALL_KEY;if(provider==='thesportsdb')return true;if(provider==='sportmonks')return!!process.env.SPORTMONKS_TOKEN;if(provider==='football-data')return!!process.env.FOOTBALL_DATA_TOKEN;return false}
export async function resolvePlayerIdentity(input={}){
  const name=clean(input.name);if(name.length<3)throw new Error('Nome insuficiente para resolver identidade.');const season=String(input.season||new Date().getFullYear());let providers=Array.isArray(input.providers)?input.providers:clean(input.provider)?[clean(input.provider)]:['api-football','thesportsdb','sportmonks'];providers=[...new Set(providers.filter(p=>['api-football','thesportsdb','sportmonks'].includes(p)))];const all=[],errors=[];
